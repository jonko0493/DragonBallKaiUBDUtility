...
namespace DragonBallKaiUBDLib
{
    ...
    public static class IO
    {
        ...
        public static int FindIndexOfSequence<T>(this IEnumerable<T> enumerable, IEnumerable<T> sequence)
        {
            int enumLoc = 0;
            int currentSeqIndex = 0;
            foreach (T item in enumerable)
            {
                if (item.Equals(sequence.ElementAt(currentSeqIndex)))
                {
                    currentSeqIndex++;
                    if (currentSeqIndex == sequence.Count())
                    {
                        return enumLoc;
                    }
                }
